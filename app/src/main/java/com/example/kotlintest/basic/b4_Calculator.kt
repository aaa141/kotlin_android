...
class b4_Calculator : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_b4_calculator)
    ...
    private fun numberButtonClicked(number: String) {
        ...
        if (expressionText.isNotEmpty() && expressionText.last().length >=15){
            ...
            Toast.makeText(this, "0은 제일 앞에 올 수 없습니.", Toast.LENGTH_LONG).show()
            return
        }

        expressionTextView.append(number)

        //TODO resultTextView 실시간으로 계산 결과를 넣어햐 하는 기능

    }

    private fun operatorButtonClicked(operator: String) {
        if (expressionTextView.text.isEmpty()){ // 연산자가 먼저 들어온 경우 무시
            return
        }
        when{
            isOperator ->{
            ...
            else ->{ // 숫자를 입력하고 연산자를 한 번도 입력하지 않은 경우
    ...
    private fun calculateExpression():String{
        ...
        if (hasOperator.not() || expressionTexts.size != 3){
            return ""
        }else if(!expressionTexts[0].isNumber() || expressionTexts[2].isNumber().not()){
            return ""
        }
        val exp1 = expressionTexts[0].toBigInteger()
        val exp2 = expressionTexts[0].toBigInteger()
    ...
    fun historyButtonClicked(v: View) {
        ...
        Thread(Runnable {
            ...
            db.historyDao().getAll().reversed().forEach {
                val historyView = LayoutInflater.from(this).inflate(R.layout.b4_history_row, null, false)
...
//확장 함수 - 객체.xxx : 그 객체에 원래 있던 함수인 것 처럼
